...
from audynesia import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def setup_bot():
    """
    To set up bot for audynesia
    """
    try:
        await udy.connect()
        config = await udy(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == udy.session.server_address:
                if udy.session.dc_id != option.id:
                    LOGS.warning(
                        f"Fixed DC ID in session from {udy.session.dc_id}"
                        f" to {option.id}"
                    )
                udy.session.set_dc(option.id, option.ip_address, option.port)
                udy.session.save()
                break
        bot_details = await udy.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await udy.start(bot_token=Config.TG_BOT_USERNAME)
        udy.me = await udy.get_me()
        udy.uid = udy.tgbot.uid = utils.get_peer_id(udy.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(udy.me)
        ...
        with open(name) as f:
            ...
            try:
                if shortname.replace(".py", "") not in Config.NO_LOAD:
                    ...
                    while flag:
                        try:
                            load_module(
                                ...
                                plugin_path=f"audynesia/{folder}",
                            )
                            break
                    ...
                    os.remove(Path(f"audynesia/{folder}/{shortname}.py"))
            except Exception as e:
                os.remove(Path(f"audynesia/{folder}/{shortname}.py"))
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await udy.get_entity(BOTLOG_CHATID)
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await udy.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
