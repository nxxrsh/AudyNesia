...
autopic_path = os.path.join(os.getcwd(), "audynesia", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "audynesia", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "audynesia", "photo_pfp.png")
...
async def autopicloop():
    ...
    while AUTOPICSTART:
        ...
        file = await udy.upload_file(autophoto_path)
        try:
            await udy(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await udy.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await udy(
                    functions.photos.DeletePhotosRequest(
                        await udy.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await udy(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await udy(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await udy(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
udy.loop.create_task(autopfp_start())
udy.loop.create_task(autopicloop())
udy.loop.create_task(digitalpicloop())
udy.loop.create_task(bloom_pfploop())
udy.loop.create_task(autoname_loop())
udy.loop.create_task(autobio_loop())
udy.loop.create_task(custompfploop())
