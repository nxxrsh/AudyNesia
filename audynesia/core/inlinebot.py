...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**AudyNesia"):
            ...
            builder = event.builder
            secret = os.path.join("./audynesia", "secrets.txt")
...
@udy.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@udy.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@udy.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@udy.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
    HELP_PIC = gvarstatus("HELP_PIC")
